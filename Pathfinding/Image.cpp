...
bool Image::ComparePixelColor(int x, int y, Color color, int tolerance) const
{
	// Cor do pixel
	...
	int difference{
		abs(int(pixelColor.r) - int(color.r)) +
		abs(int(pixelColor.g) - int(color.g)) +
		abs(int(pixelColor.b) - int(color.b)) +
		abs(int(pixelColor.a) - int(color.a))
	};

	// Compara a diferença total com o limite permitido pela tolerância.
	return difference <= ((4 * 255) * tolerance / 100);
}
